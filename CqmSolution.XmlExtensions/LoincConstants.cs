namespace CqmSolution.XmlExtensions
{
    public static class LoincConstants
    {
        public const string ALLERGIES = "48765-2";
        public const string MEDICATIONS = "10160-0";
        public const string IMMUNIZATIONS = "11369-6";
        public const string PLANOFCARE = "18776-5";
        public const string PROBLEMLIST = "11450-4";
        public const string VITALSIGNS = "8716-3";
        public const string ENCOUNTERS = "46240-8";
        public const string PROCEDURES = "47519-4";
        public const string RESULTS = "30954-2";
        public const string SOCIALHISTORY = "29762-2";
        public const string HEALTHCONCERNS = "75310-3";
        public const string GOALS = "61146-7";
        public const string INTERVENTIONS = "62387-6";
        public const string HEALTHSTATUSEVALUATIONSOUTCOMES = "11383-7";
        public const string ADVANCEDIRECTIVES = "42348-3";
        public const string FAMILYHISTORY = "10157-6";
        public const string FUNCTIONALSTATUS = "47420-5";
        public const string MEDICALEQUIPMENT = "46264-8";
        public const string INSURANCEPROVIDERS = "48768-6";
        public const string REASONFORREFERRAL = "42349-1";
        public const string REASONFORVISIT = "29299-5";
        public const string INSTRUCTIONS = "69730-0";

        public const string HOSPITAL_DISCHARGE_DIAGNOSIS = "11535-2";
        public const string ENCOUNTER_DIAGNOSIS = "29308-4";

        public const string STATUS_OBSERVATION = "33999-4";

    }
}
